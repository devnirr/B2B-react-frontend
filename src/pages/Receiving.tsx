...
export default function Receiving() {
  const [receivings] = useState([
    { id: 1, poNumber: 'PO-001', supplier: 'Supplier A', items: 50, status: 'Received' },
    { id: 2, poNumber: 'PO-002', supplier: 'Supplier B', items: 75, status: 'In Transit' },
    { id: 3, poNumber: 'PO-003', supplier: 'Supplier C', items: 30, status: 'Pending' },
  ]);
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Receiving</h1>
          <p className="text-gray-600 dark:text-gray-400">Manage incoming shipments and receiving</p>
        </div>
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
            {receivings.map((receiving) => (
              <tr key={receiving.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center gap-3">
                    <Inbox className="w-5 h-5 text-gray-400" />
                    <span className="text-sm font-medium text-gray-900 dark:text-white">{receiving.poNumber}</span>
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{receiving.supplier}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{receiving.items}</td>
                <td className="px-6 py-4 whitespace-nowrap">
