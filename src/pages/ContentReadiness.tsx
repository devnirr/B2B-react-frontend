...
export default function ContentReadiness() {
  const [content] = useState([
    { id: 1, name: 'Product Images Set A', type: 'Images', status: 'Ready', items: 25 },
    { id: 2, name: 'Marketing Copy Set B', type: 'Copy', status: 'In Review', items: 15 },
    { id: 3, name: 'Video Content Set C', type: 'Video', status: 'Draft', items: 10 },
  ]);
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Content Readiness</h1>
          <p className="text-gray-600 dark:text-gray-400">Manage content preparation and readiness</p>
        </div>
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
            {content.map((item) => (
              <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{item.items}</td>
                <td className="px-6 py-4 whitespace-nowrap">
