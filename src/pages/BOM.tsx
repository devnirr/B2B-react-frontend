...
export default function BOM() {
  const [boms] = useState([
    { id: 1, name: 'Product A BOM', items: 5, status: 'Active' },
    { id: 2, name: 'Product B BOM', items: 8, status: 'Active' },
    { id: 3, name: 'Product C BOM', items: 3, status: 'Draft' },
  ]);
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">BOM (Bill of Materials)</h1>
          <p className="text-gray-600 dark:text-gray-400">Manage product bills of materials</p>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
            {boms.map((bom) => (
              <tr key={bom.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center gap-3">
                    <Package className="w-5 h-5 text-gray-400" />
                    <span className="text-sm font-medium text-gray-900 dark:text-white">{bom.name}</span>
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{bom.items}</td>
                <td className="px-6 py-4 whitespace-nowrap">
